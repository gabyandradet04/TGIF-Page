...
function tabla(){
  ...
  var htmlList = lent.map(function(lent){
    return  "<tbody id=\'table-rows\'><tr>" +"<td>"+"<a href=\'"+ lent.url+"\'>"+lent.first_name +" "+ (lent.middle_name||"") +" "+ lent.last_name+"</a>"+"</td>" +"<td class=\'method\'>"+lent.party+"</td>" +"<td class=\'type\'>"+lent.state+"</td>" +"<td>"+lent.seniority+"</td>" +"<td>"+lent.votes_with_party_pct+" %</td>"+"</tr></tbody>"}).join("");

 var titulo = "<thead><tr> <th> Full Name </th><th>Party</th><th>State</th><th>Seniority</th><th>Percentage</tr></thead>";

    document.getElementById("senate-data").innerHTML = titulo+htmlList;
